...
존재해야 한다. Secret이 없으면 Alertmanager Pod가 Secret volume을 마운트하지 못하고
기동하지 못할 수 있다.
...
현재 저장소에서는 YAML 문법, Helm chart 렌더링, Alertmanager routing 및 Secret 파일
경로만 검증한다. 실제 Secret 생성, EKS 배포, Telegram API 호출, firing/resolved 메시지
수신, token 회전은 아직 실클러스터에서 검증하지 않은 운영 단계다.
